...
void GpsWebServer::gpsPage(EthernetClient &client, UBX_NAV_SAT_data_t *ubxNavSatData_t)
{
  ...
  client.println(F("    html += `"));
  client.println(F("      <div style='margin: 5px 0; display: flex; align-items: center;'>"));
  client.println(F("        <input type='checkbox' id='filter_${constellation.key}' "));
  client.println(F("               ${checked ? 'checked' : ''} onchange='updateDisplay()'>"));
  client.println(F("        <div style='width: 16px; height: 16px; background: ${constellation.color}; "));
  client.println(F("                    margin: 0 8px; border-radius: 2px;'></div>"));
  client.println(F("        <label for='filter_${constellation.key}' style='font-size: 12px;'>"));
  client.println(F("          ${constellation.name} ${enabled ? '(Enabled)' : '(Disabled)'}"));
  ...
  client.println(F("  document.getElementById('showUsedOnly').addEventListener('change', updateDisplay);"));
  client.println(F("  document.getElementById('showHighSignal').addEventListener('change', updateDisplay);"));
  client.println(F("  document.getElementById('zoomSlider').addEventListener('input', updateZoomLabel);"));
  client.println(F("  "));
