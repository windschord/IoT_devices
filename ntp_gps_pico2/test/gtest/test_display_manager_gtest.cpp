...
// Mock OLED Display interface
class MockOLEDDisplayInterface {
public:
    MOCK_METHOD(bool, begin, ());
    MOCK_METHOD(void, clearDisplay, ());
    MOCK_METHOD(void, setCursor, (int x, int y));
    MOCK_METHOD(void, setTextSize, (uint8_t size));
    MOCK_METHOD(void, print, (const char* text));
    MOCK_METHOD(void, println, (const char* text));
    MOCK_METHOD(void, display, ());
    MOCK_METHOD(void, setContrast, (uint8_t contrast_val));
    MOCK_METHOD(void, invertDisplay, (bool invert));
    MOCK_METHOD(void, displayOn, (bool on));
};

// Mock OLED Display HAL for testing (concrete implementation)
...
public:
    ...
    void clearDisplay() {
        for (int i = 0; i < MAX_LINES; i++) {
            std::memset(display_buffer[i], ' ', MAX_CHARS_PER_LINE);
    ...
    void print(const char* text) {
        ...
        std::strncpy(&display_buffer[cursor_y][cursor_x], text, chars_to_copy);
    ...
    bool containsText(const char* text) {
        for (int i = 0; i < MAX_LINES; i++) {
            if (std::strstr(display_buffer[i], text) != nullptr) {
                return true;
            }
        }
        return false;
...
private:
    ...
    uint32_t auto_rotation_interval = 5000;
    uint32_t last_rotation_time = 0;
    bool display_enabled = true;
    uint8_t brightness_level = 100;
    bool power_save_mode = false;
    uint32_t inactivity_timeout = 30000;
...
public:
    ExtendedDisplayManager(MockOLEDDisplay* oled) : display(oled) {
        std::memset(&cached_gps_data, 0, sizeof(cached_gps_data));
        std::memset(&cached_network_data, 0, sizeof(cached_network_data));
        std::memset(&cached_system_data, 0, sizeof(cached_system_data));
        std::memset(&cached_metrics, 0, sizeof(cached_metrics));
...
private:
    ...
    void displayTimeSync() {
        ...
        if (cached_gps_data.fix_valid) {
            ...
            std::snprintf(line, sizeof(line), "Accuracy: %.1fms", cached_metrics.ntp_accuracy_ms);
            display->println(line);
            
            std::snprintf(line, sizeof(line), "PPS Jitter: %.1fus", cached_metrics.pps_jitter_us);
            display->println(line);
            
            if (cached_metrics.clock_corrections > 0) {
                std::snprintf(line, sizeof(line), "Corrections: %d", cached_metrics.clock_corrections);
                display->println(line);
            }
        } else {
    ...
    void displayNetworkInfo() {
        ...
        if (cached_network_data.ethernet_connected) {
            ...
            std::snprintf(line, sizeof(line), "IP: %d.%d.%d.%d",
                     (ip >> 24) & 0xFF, (ip >> 16) & 0xFF,
                     (ip >> 8) & 0xFF, ip & 0xFF);
            display->println(line);
            
            std::snprintf(line, sizeof(line), "NTP: %s", 
                     cached_network_data.ntp_server_active ? "Active" : "Inactive");
            display->println(line);
            
            std::snprintf(line, sizeof(line), "Requests: %d", cached_network_data.ntp_requests);
            display->println(line);
        } else {
    ...
    void displaySystemStatus() {
        ...
        std::snprintf(line, sizeof(line), "Uptime: %dh %dm", hours, minutes);
        display->println(line);
        
        uint32_t free_kb = cached_system_data.free_memory_bytes / 1024;
        uint32_t used_kb = cached_system_data.used_memory_bytes / 1024;
        std::snprintf(line, sizeof(line), "Mem: %dK/%dK", used_kb, used_kb + free_kb);
        display->println(line);
        
        std::snprintf(line, sizeof(line), "CPU: %d%%", cached_system_data.cpu_usage_percent);
        display->println(line);
        
        std::snprintf(line, sizeof(line), "Temp: %.1fC", cached_system_data.temperature_celsius);
        display->println(line);
        
        if (cached_system_data.error_count > 0) {
            std::snprintf(line, sizeof(line), "Errors: %d", cached_system_data.error_count);
            display->println(line);
        }
    }
    ...
    void displayGNSSDetails() {
        ...
        char line[22];
        if (cached_gps_data.fix_valid) {
            std::snprintf(line, sizeof(line), "Lat: %.4f", cached_gps_data.latitude);
            display->println(line);
            
            std::snprintf(line, sizeof(line), "Lon: %.4f", cached_gps_data.longitude);
            display->println(line);
            
            std::snprintf(line, sizeof(line), "Alt: %.1fm", cached_gps_data.altitude);
            display->println(line);
            
            std::snprintf(line, sizeof(line), "Speed: %.1fkm/h", cached_gps_data.speed_kmh);
            display->println(line);
            
            std::snprintf(line, sizeof(line), "PDOP: %.1f", cached_gps_data.pdop);
            display->println(line);
        } else {
    ...
    void displayErrorStatus() {
        ...
        if (cached_system_data.error_count == 0 && display_errors == 0) {
            ...
            std::snprintf(line, sizeof(line), "System: %d", cached_system_data.error_count);
            display->println(line);
            
            std::snprintf(line, sizeof(line), "Display: %d", display_errors);
            display->println(line);
            
            std::snprintf(line, sizeof(line), "Warnings: %d", cached_system_data.warning_count);
            display->println(line);
        }
    }
    ...
    void displayPerformanceMetrics() {
        ...
        char line[22];
        std::snprintf(line, sizeof(line), "NTP Acc: %.2fms", cached_metrics.ntp_accuracy_ms);
        display->println(line);
        
        std::snprintf(line, sizeof(line), "Resp: %.1fms", cached_metrics.average_response_time_ms);
        display->println(line);
        
        std::snprintf(line, sizeof(line), "CPU 1m: %.1f%%", cached_metrics.cpu_load_1min);
        display->println(line);
        
        std::snprintf(line, sizeof(line), "Mem Frag: %d%%", cached_metrics.memory_fragmentation_percent);
        display->println(line);
    }
...
protected:
    void SetUp() override {
        mockDisplay = std::make_unique<MockOLEDDisplay>();
        displayManager = std::make_unique<ExtendedDisplayManager>(mockDisplay.get());
    }
    ...
    std::unique_ptr<MockOLEDDisplay> mockDisplay;
    std::unique_ptr<ExtendedDisplayManager> displayManager;
};

// Basic Display Tests
TEST_F(DisplayManagerTest, Initialization) {
    EXPECT_TRUE(displayManager->initialize());
    EXPECT_TRUE(displayManager->isDisplayInitialized());
    EXPECT_TRUE(displayManager->containsText("GPS NTP Server"));
}

TEST_F(DisplayManagerTest, InitializationFailure) {
    mockDisplay->simulateConnectionError(true);
    EXPECT_FALSE(displayManager->initialize());
    EXPECT_EQ(1, displayManager->getDisplayErrors());
}

TEST_F(DisplayManagerTest, ModeSwitching) {
    EXPECT_TRUE(displayManager->initialize());
    EXPECT_EQ(0, static_cast<int>(displayManager->getDisplayMode()));
    
    displayManager->nextMode();
    EXPECT_EQ(1, static_cast<int>(displayManager->getDisplayMode()));
...
// GPS Display Tests
TEST_F(DisplayManagerTest, GPSStatusNoFix) {
    EXPECT_TRUE(displayManager->initialize());
    
    GpsData gps_data = {};
    gps_data.fix_valid = false;
    gps_data.satellites_visible = 5;
    std::strcpy(gps_data.constellation_status, "GPS|GLO");
    
    displayManager->updateGPSData(gps_data);
    displayManager->setDisplayMode(DisplayMode::GPS_STATUS);
    displayManager->forceUpdate();
    
    EXPECT_TRUE(displayManager->containsText("GPS STATUS"));
    EXPECT_TRUE(displayManager->containsText("Searching"));
}

TEST_F(DisplayManagerTest, GPSStatusWithFix) {
    EXPECT_TRUE(displayManager->initialize());
    ...
    std::strcpy(gps_data.constellation_status, "GPS|GLO|GAL");
    
    displayManager->updateGPSData(gps_data);
    displayManager->setDisplayMode(DisplayMode::GPS_STATUS);
    displayManager->forceUpdate();
    
    EXPECT_TRUE(displayManager->containsText("Fix: 8/12"));
    EXPECT_TRUE(displayManager->containsText("HDOP: 1.2"));
    EXPECT_TRUE(displayManager->containsText("PPS:"));
}

// Network Display Tests
TEST_F(DisplayManagerTest, NetworkConnected) {
    EXPECT_TRUE(displayManager->initialize());
    ...
    EXPECT_TRUE(displayManager->containsText("NETWORK"));
    EXPECT_TRUE(displayManager->containsText("Ethernet: UP"));
    EXPECT_TRUE(displayManager->containsText("NTP: Active"));
}

TEST_F(DisplayManagerTest, NetworkDisconnected) {
    EXPECT_TRUE(displayManager->initialize());
    ...
    EXPECT_TRUE(displayManager->containsText("No Network"));
}

// System Display Tests
TEST_F(DisplayManagerTest, SystemStatus) {
    EXPECT_TRUE(displayManager->initialize());
    ...
    EXPECT_TRUE(displayManager->containsText("SYSTEM"));
    EXPECT_TRUE(displayManager->containsText("Uptime: 2h 1m"));
    EXPECT_TRUE(displayManager->containsText("CPU: 25%"));
    EXPECT_TRUE(displayManager->containsText("Temp: 45.5C"));
}

// Advanced Features Tests
TEST_F(DisplayManagerTest, AutoRotation) {
    EXPECT_TRUE(displayManager->initialize());
    
    displayManager->enableAutoRotation(true, 1000);  // 1 second for testing
    EXPECT_TRUE(displayManager->isAutoRotationEnabled());
    ...
    EXPECT_NE(static_cast<int>(initial_mode), static_cast<int>(final_mode));
}

TEST_F(DisplayManagerTest, BrightnessControl) {
    EXPECT_TRUE(displayManager->initialize());
    
    displayManager->setBrightness(50);
    EXPECT_EQ(50, displayManager->getBrightnessLevel());
    EXPECT_EQ(127, mockDisplay->contrast);  // ~50% of 255
    
    displayManager->setBrightness(150);  // Should clamp to 100
    EXPECT_EQ(100, displayManager->getBrightnessLevel());
}

TEST_F(DisplayManagerTest, PowerSaveMode) {
    EXPECT_TRUE(displayManager->initialize());
    
    displayManager->enablePowerSave(true, 1000);  // 1 second timeout
    EXPECT_TRUE(displayManager->isPowerSaveMode());
    ...
    EXPECT_FALSE(mockDisplay->display_on);
}

TEST_F(DisplayManagerTest, AnimationEnabled) {
    EXPECT_TRUE(displayManager->initialize());
    ...
    EXPECT_NE(nullptr, first_display);
    EXPECT_NE(nullptr, second_display);
}

// Error Handling Tests
TEST_F(DisplayManagerTest, DisplayErrors) {
    mockDisplay->simulateConnectionError(true);
    EXPECT_FALSE(displayManager->initialize());
    EXPECT_EQ(1, displayManager->getDisplayErrors());
    
    displayManager->update();
    EXPECT_GT(displayManager->getDisplayErrors(), 1);
}

TEST_F(DisplayManagerTest, ErrorStatusDisplay) {
    EXPECT_TRUE(displayManager->initialize());
    ...
    EXPECT_TRUE(displayManager->containsText("ERRORS"));
    EXPECT_TRUE(displayManager->containsText("System: 5"));
    EXPECT_TRUE(displayManager->containsText("Warnings: 12"));
}

TEST_F(DisplayManagerTest, NoErrorsStatus) {
    EXPECT_TRUE(displayManager->initialize());
    ...
    EXPECT_TRUE(displayManager->containsText("All OK"));
}

// Performance Metrics Tests
TEST_F(DisplayManagerTest, PerformanceMetrics) {
    EXPECT_TRUE(displayManager->initialize());
    ...
    EXPECT_TRUE(displayManager->containsText("PERFORMANCE"));
    EXPECT_TRUE(displayManager->containsText("NTP Acc: 0.25ms"));
    EXPECT_TRUE(displayManager->containsText("CPU 1m: 15.5%"));
}

// Parameterized Tests
class DisplayModeTest : public DisplayManagerTest, 
                       public ::testing::WithParamInterface<DisplayMode> {};

TEST_P(DisplayModeTest, AllDisplayModesWork) {
    EXPECT_TRUE(displayManager->initialize());
    ...
    DisplayMode mode = GetParam();
    displayManager->setDisplayMode(mode);
    displayManager->forceUpdate();
    
    EXPECT_GT(displayManager->getUpdateCount(), 0);
    EXPECT_NE(nullptr, displayManager->getDisplayLine(0));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    DisplayModeTest,
    ::testing::Values(
        DisplayMode::GPS_STATUS,
        DisplayMode::TIME_SYNC,
        DisplayMode::NETWORK_INFO,
        DisplayMode::SYSTEM_STATUS,
        DisplayMode::GNSS_DETAILS,
        DisplayMode::ERROR_STATUS,
        DisplayMode::PERFORMANCE_METRICS
    )
);

// Update Interval Tests
TEST_F(DisplayManagerTest, UpdateInterval) {
    EXPECT_TRUE(displayManager->initialize());
    ...
    EXPECT_LT((after_updates - initial_updates), 5);
}